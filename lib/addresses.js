...
AddressController.prototype.show = function(req, res) {
  ...
  if (self.isBlocked(req.addr)) {
    ...
    return res.jsonp({
      ...
      transactions: [],
      blocked: true,
      error: 'This address is internal to the Verus protocol and does not belong to any wallet or user. It is prevented from display here as a performance consideration.'
    });
  }
  ...
  for(var i = 0; i < addresses.length; i++) {
    ...
    if (self.isBlocked(addr)) {
      return res.status(400).jsonp({
        error: 'This address is internal to the Verus protocol and does not belong to any wallet or user. It is prevented from query here as a performance consideration.'
      });
    }
